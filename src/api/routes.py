...
def forgot_password(): 
    ...
    send_email(email, email_value, "password recover: financial app")
    return jsonify({"message": "recovery email sent"}), 200
    


@api.route("/reset-pasword/<token>", methods=["PUT"])
def reset_password(token):
    ...
    send_email(email, "password successfully reset", "password reset confirmation")
